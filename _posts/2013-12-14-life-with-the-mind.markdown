...
#### 共享表空间--->独享表空间
---
上次迁移更新数据库还是一年多前，当时自己就只负责安装复制配置数据库所需的平台环境（回想起那时都是泪啊！），至于 MySQL 的参数配置还是老大来，当时也没有考虑太周到，采取的是共享表空间方式。（时间的推移发现磁盘空间回收释放成一个问题），故决定重整调整配置开启设置使用独享表空间，方便后续对单表做表分析回收释放空间。（采用重新导出导入数据方式来做变更），主要配置修改：
